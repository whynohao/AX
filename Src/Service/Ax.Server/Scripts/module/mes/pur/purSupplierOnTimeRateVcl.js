...
purSupplierOnTimeRateVcl = function () {
...
var proto = purSupplierOnTimeRateVcl.prototype = Object.create(Ax.vcl.LibVclGrid.prototype);
proto.constructor = purSupplierOnTimeRateVcl;
proto.vclHandler = function (sender, e) {
    ...
    switch (e.libEventType) {
        case LibEventTypeEnum.Validated:
            ...
            if (e.dataInfo.fieldName != 'SUPPLIERID' && e.dataInfo.fieldName != 'SUPPLIERNAME') {
                var i;
                switch (len) {
                ...
                for (var k = 0; k < dt.data.length; k++) {
                    if (e.dataInfo.dataRow.data['SUPPLIERID'] == dt.data.items[k].data.SUPPLIERID) {
                        ...
                        e.dataInfo.dataRow.set('DELIVERYNUMBER13', drPlanQty);
                        e.dataInfo.dataRow.set('ONTIMEDELIVERYNUMBER13', drActualQty);
                        if (drPlanQty > 0 && drActualQty > 0) {
                            if (drPlanQty >= drActualQty) {
                                e.dataInfo.dataRow.set('ONTIMEDELIVERYRATE13', drActualQty / drPlanQty);
                            }

                        }
                        else {
                            e.dataInfo.dataRow.set('ONTIMEDELIVERYRATE13', 0);
                        }
                    }
                    //#endregion
                }
