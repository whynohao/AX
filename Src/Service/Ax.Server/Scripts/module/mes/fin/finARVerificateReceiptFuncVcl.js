finARVerificateReceiptFuncVcl = function () {
    Ax.vcl.LibVclDataFunc.apply(this, arguments);
};
var proto = finARVerificateReceiptFuncVcl.prototype = Object.create(Ax.vcl.LibVclDataFunc.prototype);
proto.constructor = finARVerificateReceiptFuncVcl;
proto.winId = null;
proto.fromObj = null;
var receiptData = [];
proto.doSetParam = function (vclObj) {
    ...
    var receiptRow = proto.fromObj[0].dataSet.getTable(0).data.items[0];
    receiptTableDetail = proto.fromObj[0].dataSet.getTable(1);
    for (var i = 0; i < receiptTableDetail.data.items.length; i++) {
        var tableReceiptDetailValue = receiptTableDetail.data.items[i];
        receiptData.push({
            ...
            BillNo: tableReceiptDetailValue.data["RECEIPTNO"],
            ContractNo: tableReceiptDetailValue.data["CONTRACTNO"],
        });
    }
}
    ...
    switch (e.libEventType) {
        case LibEventTypeEnum.ButtonClick:
            if (e.dataInfo.fieldName == "BtnQueryData") {
                ...
                var data = this.invorkBcf('GetReceiptData', [billDateFrom, billDateTo, contactsobjectId, contractNo,receiptData]);
                ...
                for (var i = 0; i < selectItems.length; i++) {
                    if (selectItems[i].data["ISCHOSE"] == true) {
                        records.push({
                            ...
                            CollectionAmount: selectItems[i].data["COLLECTIONAMOUNT"],
...
proto.fillData = function (returnData) {
    ...
    try {
        ...
        var grid = Ext.getCmp(this.winId + 'FINARVERIFICATERECEIPTFUNCDETAILGrid');
        var list = returnData;
        if (list !== undefined && list.length > 0) {
            for (var i = 0; i < list.length; i++) {
                ...
                newRow.set("COLLECTIONAMOUNT", info.CollectionAmount);
...
proto.fillReturnData = function (returnList) {
    ...
    try {
        ...
        for (var i = 0; i < returnList.length; i++) {
            ...
            newRow.set("PERSONID", info.PersonId);
            newRow.set("PERSONNAME", info.PersonName);
            newRow.set("RECEIPTNO", info.BillNo);
            newRow.set("CONTRACTNO", info.ContractNo);
            newRow.set("REMAINAMOUNT", info.RemainAmount);
            newRow.set("VERIFICATEAMOUNT", info.VerificateAmount);
            newRow.set("RECEIPTAMOUNT", info.CollectionAmount);
        }
    }
    finally {
        curStore.resumeEvents();//打开store事件
