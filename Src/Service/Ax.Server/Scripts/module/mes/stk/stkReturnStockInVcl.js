stkReturnStockInVcl = function () {
    Ax.vcl.LibVclData.apply(this, arguments);
};
var proto = stkReturnStockInVcl.prototype = Object.create(Ax.vcl.LibVclData.prototype);
proto.constructor = stkReturnStockInVcl;
var attId = 0;
var bodyRow;
    ...
    switch (e.libEventType) {
            ...
            if (this.isEdit) {
                if (e.dataInfo.fieldName == "BtnLoadData") {
                    ...
                    if (personId == "") {
                        ...
                        Ax.utils.LibVclSystemUtils.openDataFunc('stk.ReturnStockInQuery', '还料单数据查询', [personId, personName, this]);
                    }
                }
            }
            else {
                alert("单据只有在修改状态才能载入还料单数据！");
            }
            break;
        case LibEventTypeEnum.AddRow:
            if (e.dataInfo.tableIndex == 1) {
                ...
                var returnTime = masterRow.get('RETURNTIME');
                e.dataInfo.dataRow.set("RETURNTIME", returnTime);
            }
            break;
        case LibEventTypeEnum.Validated:
            if (e.dataInfo && e.dataInfo.tableIndex == 0) {
                this.forms[0].updateRecord(masterRow);
                var returnTime = e.dataInfo.dataRow.get('RETURNTIME');
                if (e.dataInfo.value != null) {
                    if (e.dataInfo.fieldName == "RETURNTIME") {
                        for (var i = 0; i < this.dataSet.getTable(1).data.items.length; i++) {
                            this.dataSet.getTable(1).data.items[i].set("RETURNTIME", returnTime);
                        }
                    }

                    if (e.dataInfo.fieldName == "WARE_PERSONID") {
                        //选择仓管员带出仓库
                        if (e.dataInfo.value != null && e.dataInfo.value != "") {
                            var warehouse = this.invorkBcf("SynWareHouse", [e.dataInfo.dataRow.get("WARE_PERSONID")]);
            ...
            if (e.dataInfo && e.dataInfo.tableIndex == 1) {
                ...
                var needReturnQuantity = this.invorkBcf("GetNeedReturnQuantity", [e.dataInfo.dataRow.get("FROMBILLNO"), e.dataInfo.dataRow.get("FROMROWID"), quantity]);
                e.dataInfo.dataRow.set("NEEDRETURNQUANTITY", needReturnQuantity);
