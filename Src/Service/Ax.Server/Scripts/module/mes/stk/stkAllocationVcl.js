...
stkAllocationVcl = function () {
    Ax.vcl.LibVclData.apply(this, arguments);
};
var proto = stkAllocationVcl.prototype = Object.create(Ax.vcl.LibVclData.prototype);
proto.constructor = stkAllocationVcl;
var attId = 0;
            ...
            if (this.isEdit) {
                if (e.dataInfo.fieldName == "BtnLoadData") {
                    ...
                    if (personId == "") {
                        ...
                        Ax.utils.LibVclSystemUtils.openDataFunc('stk.AllocationQuery', '调拨单数据查询', [this, personId, personName]);
                    }
                }
            }
            else {
                alert("单据只有在修改状态才能载入调拨单数据！");
            }
        case LibEventTypeEnum.Validated:
            if (e.dataInfo && e.dataInfo.tableIndex == 0) {
                switch (e.dataInfo.fieldName) {
                    case 'FROMBILLNO'://投产单带出合同号
