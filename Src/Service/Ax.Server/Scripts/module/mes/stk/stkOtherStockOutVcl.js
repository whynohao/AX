stkOtherStockOutVcl = function () {
    Ax.vcl.LibVclData.apply(this, arguments);
};
var proto = stkOtherStockOutVcl.prototype = Object.create(Ax.vcl.LibVclData.prototype);
proto.constructor = stkOtherStockOutVcl;
var attId = 0;
var bodyRow;
    ...
    switch (e.libEventType) {
            ...
            if (e.dataInfo && e.dataInfo.tableIndex == 0) {
                switch (e.dataInfo.fieldName) {
                    ...
                    case 'BATCHNO':
                        ...
                        /*
                    case 'PO_PERSONID'://人员带出部门
                        if (e.dataInfo.value != null && e.dataInfo.value != "") {
                            var field = Ext.getCmp('PO_DEPTID0_' + this.winId);
                            field.store.add({ Id: e.dataInfo.dataRow.data.DEPTID, Name: e.dataInfo.dataRow.data.DEPTNAME });
                            field.select(e.dataInfo.dataRow.data.DEPTID);
                        }
                        break;
                        */
                }
        ...
        case LibEventTypeEnum.AddRow:
            if (e.dataInfo.tableIndex == 1) {
                ...
                var productContractNo = masterRow.get('PRODUCTCONTRACTNO');
                e.dataInfo.dataRow.set("PRODUCTCONTRACTNO", productContractNo);
        ...
        case LibEventTypeEnum.ButtonClick:
            if (e.dataInfo.fieldName == "BtnBarcodePrint") {
                if (!this.isEdit) {
                    var printtype = parseInt(this.dataSet.getTable(0).data.items[0].data["STKTYPE"]) == 0 ? 2 : 3;
                    var frombillno = this.dataSet.getTable(0).data.items[0].data["BILLNO"];
                    Ax.utils.LibVclSystemUtils.openDataFunc('stk.BarcodePrintDataFunc', "条码打印", [printtype, frombillno, this.tpl.ProgId, this.tpl.DisplayText]);
                }
                else {
