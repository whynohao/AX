stkProductBackStockInVcl = function () {
    Ax.vcl.LibVclData.apply(this, arguments);
};
var proto = stkProductBackStockInVcl.prototype = Object.create(Ax.vcl.LibVclData.prototype);
proto.constructor = stkProductBackStockInVcl;
var attId = 0;
var bodyRow;
            ...
            if (this.isEdit) {
                if (e.dataInfo.fieldName == "BtnLoadData") {
                    ...
                    if (companyId == "") {
                        ...
                        this.invorkBcf("ChangeOpenType", ['来源单']);//修改打开状态
                        var type = "来源单";
                        Ax.utils.LibVclSystemUtils.openDataFunc('stk.ProductBackStockInQuery', '生产领料单数据查询', [companyId, this, type]);
                    }
                }
                if (e.dataInfo.fieldName == "BtnLoadReturnData") {
                    ...
                    if (companyId == "") {
                        ...
                        this.invorkBcf("ChangeOpenType", ['任务单']);//修改打开状态
                        var type = "任务单";
                        Ax.utils.LibVclSystemUtils.openDataFunc('stk.ProductBackStockInQuery', '生产退料任务数据查询', [companyId, this, type]);
                    }
                }
            }
            else {
                alert("单据只有在修改状态才能载入生产退料数据！");
            }
            break;
