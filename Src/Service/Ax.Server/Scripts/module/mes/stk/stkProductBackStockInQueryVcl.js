...
stkProductBackStockInQueryVcl = function () {
    Ax.vcl.LibVclDataFunc.apply(this, arguments);
};
var proto = stkProductBackStockInQueryVcl.prototype = Object.create(Ax.vcl.LibVclDataFunc.prototype);
proto.constructor = stkProductBackStockInQueryVcl;
    ...
    try {
        ...
        if (list !== undefined && list.length > 0) {
            for (var i = 0; i < list.length; i++) {
                ...
                newRow.set('FROMBILLNO', info.FrombillNo);//来源生产领料出库单号
...
function fillStockInData(returnData) {
    debugger;
    var grid = Ext.getCmp(proto.winId + 'STKPRODUCTBACKINDETAILGrid')
    var list = returnData;
    Ext.suspendLayouts();
    ...
    try {
        if (list !== undefined && list.length > 0) {
            for (var i = 0; i < list.length; i++) {
                ...
                newRow.set('TASKNO', info.TaskNo);//来源收货单号
                newRow.set('FROMBILLNO', info.FrombillNo);//来源生产领料出库单号
                newRow.set('FROMROWID', info.FromRowId);// 来源行标识
                newRow.set('WAREHOUSEID', info.WarehouseId);// 仓库 空
                newRow.set('WAREHOUSENAME', info.WarehouseName);// 仓库名称  空
        ...
        case LibEventTypeEnum.ButtonClick:
            ...
            if (e.dataInfo.fieldName == "BTNSELECT") {
                if (openType == "来源单") {
                    ...
                    var materialId = this.dataSet.getTable(0).data.items[0].data['MATERIALID'];//物料ID
                    var materialTypeId = this.dataSet.getTable(0).data.items[0].data['MATERIALTYPEID'];//物料类别ID
                    var workshopSectionId = this.dataSet.getTable(0).data.items[0].data['WORKSHOPSECTIONID'];//工段
                    if (companyId == "" || companyId == undefined) {
                        alert("公司不能为空！");
                        return;
                    }
                        ...
                        var materialId = this.dataSet.getTable(0).data.items[0].data['MATERIALID'] == undefined ? "" : this.dataSet.getTable(0).data.items[0].data['MATERIALID'];
                        var materialTypeId = this.dataSet.getTable(0).data.items[0].data['MATERIALTYPEID'] == undefined ? "" : this.dataSet.getTable(0).data.items[0].data['MATERIALTYPEID'];
                        var workshopSectionId = this.dataSet.getTable(0).data.items[0].data['WORKSHOPSECTIONID'] == undefined ? "" : this.dataSet.getTable(0).data.items[0].data['WORKSHOPSECTIONID'];
                        //if (fromRowId == 0) {
                        //    fromRowId = "";
                        //}
                        //this.win.close();
                        var data = this.invorkBcf('GetData', [companyId, contactObjectId, "", materialId, materialTypeId, workshopSectionId]);
                        fillData.call(this, data);
                    }
                }
                break;
