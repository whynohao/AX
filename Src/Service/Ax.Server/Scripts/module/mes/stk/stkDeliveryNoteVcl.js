stkDeliveryNoteVcl = function () {
    Ax.vcl.LibVclData.apply(this, arguments);
}
var proto = stkDeliveryNoteVcl.prototype = Object.create(Ax.vcl.LibVclData.prototype);
proto.constructor = stkDeliveryNoteVcl;
        ...
        if (e.dataInfo.fieldName == "BtnGetData") {
            ...
            if (!this.isEdit) {
                ...
                if (currentState != "1") {
                    ...
                    backData = this.invorkBcf('BuildQualityCheck', [deliverylNo]);
                }
                if (backData != null) {
                    var isExist = backData.IsExist;
                    if (isExist) {
                        Ext.Msg.show({
                            ...
                            fn: function (h) {
                                if (h == "yes") {
                                    var billNoList = backData.BillNo.split(',');
                                    for (var i = 0; i < billNoList.length; i++) {
                                        var curPks = [];
                                        curPks.push(billNoList[i]);
                                        Ax.utils.LibVclSystemUtils.openBill('qc.TaskList', BillTypeEnum.Bill, "质检任务单", BillActionEnum.Browse, undefined, curPks);
                                    }
                                }
                            }
                        })
