comSimSaleBOMView = function () {
    Ax.tpl.LibBillTpl.apply(this, arguments);
    if (this.vcl.funcView.containsKey("default")) {
        this.vcl.funcView.get("default").name = "onReady";
    }
};
var proto = comSimSaleBOMView.prototype = Object.create(Ax.tpl.LibBillTpl.prototype);
proto.constructor = comSimSaleBOMView;
proto.onReady = function (billAction, curPks, isF4, lookVersionObj, changeView) {
    ...
    //    function SetcmbData() {
    //        var data = [];
    //        for (var i = 1; i <= vcl.levelCount; i++) {
    //            data.push([i, [i]]);
    //        }
    //        return data;
    //    }
    //}
    //展开到某一层级
    //function expendTreeForLevel(level, childNodes) {
    //    for (var i = 0; i < childNodes.length; i++) {
    //        if (childNodes[i].data["BOMLEVEL"] < level) {
    //            childNodes[i].expand(false);
    //            if (childNodes[i].childNodes.length > 0) {
    //                expendTreeForLevel(vcl.level, childNodes[i].childNodes);
    //            }
    //        }
    //    }
    //}
    ...
    //        handler: function () {
    //            //vcl.tree.expandAll();
    //            //var rootNode = vcl.tree.getSelectionModel().getSelected().items[0];
    //            //console.log(rootNode);
    //            //rootNode.expandChildren();
    //            vcl.level = levelComBo.getValue();
    //            var rootNode = vcl.tree.getRootNode();
    //            rootNode.collapse(true);
    //            rootNode.expand(false);
    //            expendTreeForLevel.call(this, vcl.level, rootNode.childNodes);
    //        }
    //    });
    //    return btnRefresh;
    //}
    ...
    if (vcl.tpl.Layout.GridRange != null) {
        bodyPanel = Ext.create('Ext.panel.Panel', {
            ...
            region: "center",
            //tbar: [
            //    createlevelComBo.call(),
            //    createRefreshBtn.call(this)
            //]
        });
...
proto.createTree = function (gridInfo) {
    ...
    var treePanel = Ext.create('Ext.tree.Panel', {
        ...
        animate: true,
        //rootVisible: true,
        //bodyStyle: "background-color:#C2FF68",
        ...
        viewConfig: {
            ...
            listeners: {
                beforedrop: function (node, data, overModel, dropPosition, dropHandler, eOpts) {
                    if (vcl.isEdit === false) {
                        Ext.Msg.alert("修改状态下才可以移动节点!");
                        return false;
                    }
                    //else {
                    //    if (data.records[0].parentNode.data.children.length === 0) {
                    //        data.records[0].parentNode.data.iconCls = "leaf";
                    //    }
                    //    if (dropPosition === "append") {
                    //        overModel.data.iconCls = "tree-open";
                    //    }
                    //};
                },
        ...
        listeners: {
            //展开节点时，更改父节点图标样式 
            //beforeitemexpand: function (eOpts,node) {
            //    alert(11);
            //    if (node.childNodes.length > 0) {
            //        node.data.iconCls = "tree-open";
            //    }
            //    for (var i = 0; i < node.childNodes.length; i++) {
            //        var curChild = node.childNodes[i];
            //        if (curChild.childNodes == "") {
            //            curChild.data.iconCls = "leaf";
            //        }
            //        else {
            //            curChild.data.iconCls = "tree";
            //        }
            //    }
            //},
            //beforeitemcollapse: function (eOpts,node) {
            //    alert(11);
            //    if (node.childNodes.length > 0) {
            //        node.data.iconCls = "tree";
            //    }
            //},
                                ...
                                handler: function () {
                                    ...
                                    if (record.data["ROW_ID"] != "") {
                                        ...
                                        for (var i = 0; i < rowids.length; i++) {
                                            //for (var j = 0; j < vcl.dataSet.getTable(1).data.length; j++) {
                                            //    if (vcl.dataSet.getTable(1).data.items[j].data["ROW_ID"] == rowids[i]) {
                                            //        vcl.dataSet.getTable(1).data.remove(vcl.dataSet.getTable(1).getAt(j));
                                            //        break;
                                            //    }
                                            //}
