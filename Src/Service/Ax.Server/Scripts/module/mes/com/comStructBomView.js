...
var proto = comStructBomView.prototype = Object.create(Ax.tpl.LibBillTpl.prototype);
proto.constructor = comStructBomView;
...
proto.createTree = function (gridInfo) {
    ...
    var treePanel = Ext.create('Ext.tree.Panel', {
        ...
        listeners: {
            ...
            edit: function (obj, e) {
                var me = this;
                if (e.field == "MATCHTYPE") {//选配类型
                    if (e.value == 0) {
                        e.grid.getView().getRow(e.rowIdx).style.backgroundColor = '#FFFFFF';
                    }
                    else if (e.value == 1) {
                        e.grid.getView().getRow(e.rowIdx).style.backgroundColor = '#2894FF';
                    }
                    else if (e.value == 2) {
                        e.grid.getView().getRow(e.rowIdx).style.backgroundColor = '#B15BFF';
                    }
                }
                else if (e.field == "ISAUTOMATCHMAT") {//自动匹配
                    if (e.value == 1) {
                        e.grid.getView().getRow(e.rowIdx).style.backgroundColor = '#CCFF99';
                    }
                    else if (e.value == 0) {
                        e.grid.getView().getRow(e.rowIdx).style.backgroundColor = '#FFFFFF';
                    }
                }
                else if (e.field == "SUBMATERIALID") {//虚拟件
                    if (e.record.data["MATERIALTYLE"] == 3) {
                        e.grid.getView().getRow(e.rowIdx).style.backgroundColor = '#F0F0F0';
                    }
                    else if (e.record.data["MATERIALTYLE"] == 0) {
                        e.grid.getView().getRow(e.rowIdx).style.backgroundColor = '#FFFFFF';
                    }
                    else if (e.record.data["MATERIALTYLE"] == 2) {//通用件
                        e.grid.getView().getRow(e.rowIdx).style.backgroundColor = '#FFB5B5';
                    }
                    else if (e.record.data["MATERIALTYLE"] == 0) {
                        e.grid.getView().getRow(e.rowIdx).style.backgroundColor = '#FFFFFF';
                    }
                        ...
                        items: [{
                            ...
                            handler: function () {
                                ...
                                for (var j = deleteRows.length - 1; j >= 0; j--) {
                                    ...
                                    items = vcl.dataSet.getTable(5).data.items;
                                    for (var i = items.length - 1; i >= 0; i--) {
                                        if (record.get("ROW_ID") == items[i].get("PARENTROWID")) {
                                            //vcl.dataSet.getTable(5).remove(items[i]);
                                            vcl.deleteRow(5, items[i]);

                                        }
                                    }

                                    items = vcl.dataSet.getTable(4).data.items;
                                    for (var i = items.length - 1; i >= 0; i--) {
                                        if (record.get("ROW_ID") == items[i].get("PARENTROWID")) {
                                            //vcl.dataSet.getTable(4).remove(items[i]);
                                            vcl.deleteRow(4, items[i]);
                                        }
                                    }

                                    var items = vcl.dataSet.getTable(3).data.items;
                                    for (var i = items.length - 1; i >= 0; i--) {
                                        if (record.get("ROW_ID") == items[i].get("GRANDPARENTROWID")) {
                                            //vcl.dataSet.getTable(3).remove(items[i]);
                                            vcl.deleteRow(3, items[i]);
                                        }
                                    }
                                    ...
                                    items = vcl.dataSet.getTable(6).data.items;
                                    for (var i = items.length - 1; i >= 0; i--) {
                                        if (record.get("ROW_ID") == items[i].get("GRANDPARENTROWID")) {
                                            vcl.dataSet.getTable(6).remove(items[i]);
                                            //vcl.deleteRow(6, items[i]);
                                        }
                                    }
                            ...
                            handler: function () {
                                var struecBom = vcl.getSaleBomID(currNode.data["SUBMATERIALID"]);
