...
EmLineSameQuarterKPIView = function () {
    
    Ax.tpl.LibRptTpl.apply(this, arguments);
    this.vcl.funcView.add('createChart', { name: 'createChart', display: 'KPI图表' });
};
var proto = EmLineSameQuarterKPIView.prototype = Object.create(Ax.tpl.LibRptTpl.prototype);
proto.constructor = EmLineSameQuarterKPIView;
    ...
    build: function (view, isRate, rateField) {
        ...
        dt.each(function (rec) {
            if (idx < 10) {
                ...
                var quarter = rec.get("QUARTER");
                quarter = Ext.isEmpty(quarter) ? "季度" : quarter;
                switch(quarter)
                {
                    case 0: quarter = "一季度";
                        break;
                ...
                series.push({
                    ...
                    tips: {
                        ...
                        renderer: function (storeItem, item) {
                            ...
                            var html = "<div>产线：" + producelinename + "<br/>年份：" + time + "<br/>季度：" + quarter + "<br/>OEE：" + parseFloat(v * 100).toFixed(2) + "%</div>";
                            this.setTitle(html);
                        }
                    },
                    title: producelinename + quarter
                });
            }
        });
