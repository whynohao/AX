...
EmLineSameYearKPIView = function () {

    Ax.tpl.LibRptTpl.apply(this, arguments);
    this.vcl.funcView.add('createChart', { name: 'createChart', display: 'KPI图表' });
};
var proto = EmLineSameYearKPIView.prototype = Object.create(Ax.tpl.LibRptTpl.prototype);
proto.constructor = EmLineSameYearKPIView;
    ...
    build: function (view, isRate, rateField) {
        ...
        dt.each(function (rec) {
            if (idx < 10) {
                ...
                var rptyear = rec.get("RPTYEAR");
                rptyear = Ext.isEmpty(rptyear) ? "年份" : rptyear;
                var name = 'data' + idx;
                idx++;
                ...
                series.push({
                    ...
                    tips: {
                        ...
                        renderer: function (storeItem, item) {
                            ...
                            var html = "<div>产线：" + producelinename + "<br/>年份：" + rptyear + "<br/>季度：" + time + "<br/>OEE：" + parseFloat(v * 100).toFixed(2) + "%</div>";
                            this.setTitle(html);
                        }
                    },
                    title: producelinename + "-" +rptyear
                });
            }
        });
