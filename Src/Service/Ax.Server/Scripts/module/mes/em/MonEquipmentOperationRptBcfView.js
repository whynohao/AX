MonEquipmentOperationRptBcfView = function () {
    Ax.tpl.LibDailyRptTpl.apply(this, arguments);
    this.vcl.funcView.add('proto.createChart', { name: 'createChart', display: '图表' });
};

var proto = MonEquipmentOperationRptBcfView.prototype = Object.create(Ax.tpl.LibDailyRptTpl.prototype);
proto.constructor = MonEquipmentOperationRptBcfView;
...
proto.sysKPIChart = {
    build: function (view) {
        ...
        var checkDate = "";
        if (rowNum > 0) {
            checkDate = dt.data.items[0].data["CREATEDATE"].toString();
        }
        checkDate = checkDate.substr(0, 4) + '年' + checkDate.substr(4, 2) + '月' + checkDate.substr(6, 2) + '日';
        for (var i = 0; i <= rowNum - 1; i++) {
          
            list.push({
                'pointId': dt.data.items[i].data["EQUIPMENTNAME"],
               // 'abnormalcount': dt.data.items[i].data["ABNORMALCOUNT"],
                'normalcount': dt.data.items[i].data["BOOTTIME"]
            });
        }


        this.myDataStore = Ext.create('Ext.data.JsonStore', {
            fields: ['pointId',  'normalcount'],
            data: list
        });
        ...
        var chart = Ext.create('Ext.chart.Chart', {
            ...
            items: [
            {
                type: 'text', text: '设备运行状态日报表', font: '22px Helvetica',
                width: 100, height: 30,
                x: 450, y: 12
            },
             {
                 type: 'text', text: '(分钟)', font: '12px Helvetica',
                 width: 100, height: 30,
                 x: 50, y: 20
             }
             ,
             {
                 type: 'text', text: checkDate, font: '12px Helvetica',
                 width: 100, height: 30,
                 x: 700, y: 20
             }
            ],
        ...
        var select = Ext.create(Ext.Action, {
            ...
            hidden: vcl.isEdit,
            handler: function () {
                Ax.utils.LibQueryForm.createForm(vcl, "Mon.EquipmentOperationRptBcf", undefined, function () {
                    vcl.win.removeAll();
