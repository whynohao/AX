...
{
    public class DMUserPermissionCache: MemoryCache
    {
        ...
        private void LibHandleCache_NewLogin(AxCRL.Core.Comm.LibHandle newHandle)
        {
            ...
            lock (_LockObj)
            {
                if (this.Contains(newHandle.PersonId))
                {
                    this.Remove(newHandle.PersonId);
                }
            }
        }
        public static new DMUserPermissionCache Default
        {
            get
            {
                if (_Default == null)
                {
                    lock (_LockObj)
                    {
                        if (_Default == null)
                        {
                            _Default = new DMUserPermissionCache("LibRolePermissionCache");
