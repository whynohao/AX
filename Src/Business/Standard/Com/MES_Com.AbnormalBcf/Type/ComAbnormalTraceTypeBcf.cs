...
namespace MES_Com.AbnormalBcf
{
    ...
    /// 异常追踪单单据类型
    /// </summary>
    [ProgId(ProgId = "com.AbnormalTraceType", ProgIdType = ProgIdType.Bcf, VclPath = @"/Scripts/module/mes/com/comAbnormalTraceTypeVcl.js")]
    public class ComAbnormalTraceTypeBcf : LibBcfData
    {
        /// <summary>
        /// 异常追踪单单据类型 模板注册
        /// </summary>
        /// <returns>返回 异常追踪单单据类型 的数据模板</returns>
        protected override LibTemplate RegisterTemplate()
        {
            return new ComAbnormalTraceTypeBcfTemplate("com.AbnormalTraceType");
        }

        /// <summary>
        /// 异常追踪单单据类型 保存前验证【不能出现相同的使用条件;相同的使用条件中不能出现相同的人；发短信或发微信时，接收人的电话或微信须存在；不被异常追踪单引用】
        /// </summary>
        protected override void BeforeUpdate()
        {
            base.BeforeUpdate();
            #region 当异常追踪单单据类型变更时，确保原始的异常追踪单单据类型不被异常追踪单引用
            ...
            if (masterRow.HasVersion(DataRowVersion.Original))
            {
                ...
                {
                    string sql = string.Format("SELECT COUNT(BILLNO) FROM COMABNORMALTRACE WHERE TYPEID={0}", LibStringBuilder.GetQuotString(typeId));
                    int count = LibSysUtils.ToInt32(this.DataAccess.ExecuteScalar(sql));
                    if (count > 0)
                    {
                        this.ManagerMessage.AddMessage(LibMessageKind.Error, string.Format("异常追踪单单据类型{0}在异常追踪单被引用，无法修改", typeId));
                    }
                }
            }
            #endregion

            #region 业务处理流程明细表中不能出现相同的使用条件;相同的使用条件中不能出现相同的人；发短信或发微信时，接收人的电话或微信须存在；
            ...
            foreach (DataRow curRow in subTable.Rows)
            {
                ...
                {
                    this.ManagerMessage.AddMessage(LibMessageKind.Error, string.Format("业务处理流程规则配置中行标识{0}的使用条件存在重复", LibSysUtils.ToInt32(curRow["ROW_ID"])));
                }
                else
                {
                ...
                if (childRows != null)
                {
                    foreach (DataRow childRow in childRows)
                    {
                        ...
                        {
                            this.ManagerMessage.AddMessage(LibMessageKind.Error, string.Format("业务处理流程规则中行标识{0}下业务处理流程明细中行标识{1}的处理人存在重复",
                        ...
                        {
                            this.ManagerMessage.AddMessage(LibMessageKind.Error, string.Format("业务处理流程规则中行标识{0}下业务处理流程明细中行标识{1}的处理人的微信不存在，无法发微信",
                                LibSysUtils.ToInt32(childRow["PARENTROWID"]), LibSysUtils.ToInt32(childRow["ROW_ID"])));
                        }
                        if (string.IsNullOrWhiteSpace(phoneNo) && needSms)
                        {
                            this.ManagerMessage.AddMessage(LibMessageKind.Error, string.Format("业务处理流程规则中行标识{0}下业务处理流程明细中行标识{1}的处理人的电话不存在，无法发短信",
                                LibSysUtils.ToInt32(childRow["PARENTROWID"]), LibSysUtils.ToInt32(childRow["ROW_ID"])));
                        }
                    }
                }
        ...
        /// <summary>
        /// 异常追踪单单据类型 删除前验证【不被异常追踪单引用】
        /// </summary>
        protected override void BeforeDelete()
        {
            base.BeforeDelete();
            #region 当异常追踪单单据类型删除时，确保原始的异常追踪单单据类型不被异常追踪单引用
            DataRow masterRow = this.DataSet.Tables[0].Rows[0];
            if (masterRow.HasVersion(DataRowVersion.Original))
            {
                string typeId = LibSysUtils.ToString(masterRow["TYPEID", DataRowVersion.Original]);
                if (LibSysUtils.ToString(masterRow["TYPEID"]).CompareTo(typeId) != 0)
                {
                    ...
                    if (count > 0)
                    {
                        this.ManagerMessage.AddMessage(LibMessageKind.Error, string.Format("异常追踪单单据类型{0}在异常追踪单被引用，无法删除", typeId));
                    }
                }
            }
            #endregion
    ...
    /// 异常追踪单单据类型 数据模板
    /// </summary>
    public class ComAbnormalTraceTypeBcfTemplate : LibTemplate
    {
        // 异常追踪单单据类型 主表
        private const string tableName = "COMABNORMALTRACETYPE";
        // 异常追踪单单据类型 子表 业务处理流程规则配置
        private const string bodyTableName = "COMABNORMALTRACETYPEDETAIL";
        // 异常追踪单单据类型 业务处理流程规则配置 子子表 业务处理流程明细
        private const string subTableName = "COMABNORMALTRACETYPEFLOW";

        /// <summary>
        /// 异常追踪单单据类型 模板功能定义
        /// </summary>
        /// <param name="progId">异常追踪单单据类型 功能标识</param>
        public ComAbnormalTraceTypeBcfTemplate(string progId)
            : base(progId, BillType.Master, "异常追踪单单据类型")
        {
        }
        ...
        protected override void BuildDataSet()
        {
            ...
            DataSourceHelper.AddColumn(new DefineField(masterTable, "NEEDSOLUTION", "解决措施为必填") { DataType = LibDataType.Boolean, ControlType = LibControlType.YesNo, DefaultValue = true });
            ...
            #region 异常追踪单单据类型 子表 业务处理流程规则配置
            ...
            DataSourceHelper.AddColumn(new DefineField(bodyTable, "PROCESSDETAIL", "业务处理流程明细") { DataType = LibDataType.Boolean, ControlType = LibControlType.YesNo, ReadOnly = true, SubTableIndex = 2 });
            ...
            #region 异常追踪单单据类型 业务处理流程规则配置 子子表 业务处理流程明细
            ...
            DataSourceHelper.AddColumn(new DefineField(subTable, "PROCESSLEVEL", "处理层级") { DefaultValue = 1, DataType = LibDataType.Int32, ControlType = LibControlType.Number, QtyLimit = LibQtyLimit.GreaterThanZero });
            DataSourceHelper.AddColumn(new DefineField(subTable, "PERSONID", "处理人代码", FieldSize.Size20)
            {
                #region 人员
                AllowEmpty = false,
        ...
        protected override void DefineViewTemplate(DataSet dataSet)
        {
            ...
            layout.HeaderRange = layout.BuildControlGroup(0, string.Empty, new List<string>() { "TYPEID", "TYPENAME", "NEEDSOLUTION", "DEFAULTCREATESTATE" });
            layout.TabRange.Add(layout.BuildGrid(1, "业务处理流程规则配置"));
            layout.SubBill.Add(2, layout.BuildGrid(2, "业务处理流程明细"));
