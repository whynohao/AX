...
namespace Jikon.MES_Dm.DocumentDataBcf
{
    ...
    public class DmDocumentBcf : LibBcfData, ILibLiveUpdate
    {
        ...
        public MoveAttachResult MoveAttach(LibAttachData attachData)
        {
            ...
            {
                DMUserPermission permission = DMUserPermissionCache.Default.Get<DMUserPermission>(OrignUserHandle.PersonId);
                if (permission != null)
                {
                    permission.AddPermission(false, docId, (int)DMFuncPermissionEnum.Manage);
                }
                //因采用了Redis缓存，不再时内存对象的二进制缓存，必须重新Set回内存才能实现后续提取本对象时的生效 Zhangkj20170227
                DMUserPermissionCache.Default.Set<DMUserPermission>(OrignUserHandle.PersonId, permission, new TimeSpan(0, 30, 0));
            }
        ...
        private void DealFileInfo(DataRow masterRow,bool isAddNew)
        {
            ...
            if (string.IsNullOrEmpty(fileName) == false)
            {
                ...
                masterRow["DOCTYPE"] = fileInfo.Extension.ToLower();
