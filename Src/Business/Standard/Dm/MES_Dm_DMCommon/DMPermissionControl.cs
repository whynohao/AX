...
namespace Jikon.MES_Dm.DMCommon
{
    ...
    public class DMPermissionControl
    {
        ...
        {
            bool ret = false;
            if (handle.Handle == LibHandleCache.Default.GetSystemHandle().Handle||string.IsNullOrEmpty(handle.UserId)||handle.UserId=="admin")
            {
                ret = true;
            }
            else
        ...
        public List<DMFuncPermissionEnum> GetPermissionOf(string userHandle,string docId)
        {
            ...
            if (libHandle.UserId.Equals("admin") || (DirTypeEnum)dirLink.DirType == DirTypeEnum.Private||libHandle.Handle==LibHandleCache.Default.GetSystemHandle().Handle)
